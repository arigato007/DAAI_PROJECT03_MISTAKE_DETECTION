...
class Config(object):
    ...
    segment_features_directory: str = "/content/drive/MyDrive/Mistake_Detection/features"

    ckpt_directory: str = "/content/drive/MyDrive/Mistake_Detection/error_recognition_main/checkpoints"
...
def eval_er(config, threshold):
    ...
    _,test_sub_step_metrics, test_step_metrics = test_er_model(model, test_loader, criterion, config.device, phase="test", step_normalization=True, sub_step_normalization=True, threshold=threshold)

    out_dir = "results_evaluation"
    os.makedirs(out_dir, exist_ok=True)
    fname = f"{config.task_name}_{config.split}_{config.backbone}_{config.variant}.xlsx"
    out_path = os.path.join(out_dir, fname)

    df = pd.DataFrame([
        {"level": "test Sub Step Level Metrics", **test_sub_step_metrics, "threshold": threshold},
        {"level": "test Step Level Metrics",     **test_step_metrics,     "threshold": threshold},
    ])
...
if __name__ == "__main__":
    ...
    if args.task_name == const.ERROR_CATEGORY_RECOGNITION:
        ...
        # Convert error_category to match dataset's internal naming convention
        if conf.error_category is not None:
            conf.error_category = conf.error_category.replace(" ", "")
    else:
