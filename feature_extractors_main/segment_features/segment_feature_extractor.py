...
def extract_features(video_data_raw, feature_extractor, transforms_to_apply, method):
    ...
    if method == "perception_encoder":
        ...
        if not isinstance(video_data_raw, torch.Tensor):
            video_data_raw = torch.from_numpy(video_data_raw)

        T = video_data_raw.shape[1]
        mid_idx = T // 2
        frame = video_data_raw[:, mid_idx]

        frame = frame.permute(1, 2, 0).cpu().numpy()
        frame = np.clip(frame, 0, 255).astype(np.uint8)
        frame = Image.fromarray(frame).convert("RGB")

        frame_tensor = transforms_to_apply(frame).unsqueeze(0).to(device)

        with torch.no_grad():
            with torch.cuda.amp.autocast(enabled=frame_tensor.is_cuda):
                features, _, _ = feature_extractor(frame_tensor)
    ...
    if method in ["omnivore"]:
        ...
        raise ValueError(f"Unsupported backbone {method}")
...
# Main
def main():
    video_files_path = "/content/drive/MyDrive/Mistake_Detection/captain_cook_4d_gopro_resized"
